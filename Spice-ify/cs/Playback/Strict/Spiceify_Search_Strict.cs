...
{
    public static bool checkEquality<T>(T[] splittedInput, T[] splittedArtistSong)
    {
        return Enumerable.SequenceEqual(splittedInput, splittedArtistSong);
    }
    ...
    public bool Execute()
    {
        ...
        //    string id = root.tracks.items[0].id.ToString();
        string albumImgUrl = root.tracks.items[0].album.images[0].url.ToString();
        CPH.SetGlobalVar("albumImgUrl", albumImgUrl, true);
        string input = args["rawInput"].ToString().ToLower();
        string[] splittedInput = input.Split(' ');
        string artistSong = (artistName.ToLower() + " " + songName.ToLower());
        string[] splittedArtistSong = artistSong.Split(' ');
        ...
        bool abortSR;
        if (checkEquality(splittedInput, splittedArtistSong))
        {
        ...
        {
            CPH.SendMessage("your query must match a result");
            abortSR = true;
            CPH.SetGlobalVar("abortSR", abortSR, false);
        }
